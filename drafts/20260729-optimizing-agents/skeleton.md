...
**The thread:** once you build your own agent, it will sometimes be confidently wrong. Rung 2 is two jobs: make that rarer, and know where it happens.

15. At rung 2 you build the agent yourself on a vendor's model, so you now own the loop, including when it stops and asks a person.
    - Retrieval lives here: it became something the agent does rather than something done to it [outline].

16. Every run ends one of three ways: right, escalated, or wrong without escalating. Teams count the second, and the third decides whether the agent pays.
    - Three-outcome model [BV §2b].

17. On legal form extraction, the same agent saves a million dollars a year or loses money, depending only on what one silent error costs downstream.
...
18. Minimising human intervention, the metric most teams track, pushes you straight into those silent errors.
    - At full coverage the three cases save 70%, cost 182% more, and cost 2,702% more [BV §2b].

19. So rung 2 is two jobs: make the agent wrong less often, then decide how much to let it run unsupervised.
    - More reliability pays more as stakes rise, because it opens work you could not trust it with before: at $3,000 an error, today's agent earns 10% coverage and a tenfold better one earns 50% [BV §2b].

*Job 1: make it wrong less often.*

20. Most failures trace to what the agent knew, or failed to know, at the moment it acted.
...
21. The biggest efficiency gain comes from using the default tools better, not from building more of them.
    - Code execution over tools: 150,000 tokens to 2,000 [V, verified]. Defaults are co-trained with the model, so replacing them costs capability [V].

22. Use several agents only when one has plateaued, and expect many agents sharing state to coordinate whether you designed it or not.
...
*Job 2: know where it is wrong.*

23. You need a signal that separates right from confidently wrong, and the trustworthy ones come from components agreeing and from hard checks, not from asking the model.
    - Six methods, cheapest first [BV §2d]. Verbalized confidence now ranks better than logprobs on post-2025 models but is not well calibrated [BV §2d, verified].

24. Calibrating that signal takes a few hundred labelled examples, and your escalation queue is already producing them.
    - Every escalated item gets a human verdict, which is a label [BV §2d].

25. Your checks can be wrong too, in both directions.
    - Public benchmarks: 61.1% of SWE-bench samples flagged for tests that reject valid solutions; 345 wrong patches recorded as passing; SWE-bench Pro graders 8.5% false positive, 24% false negative [V, audited].
    - First-hand: loose checks that agents slipped past, strict ones that failed correct answers [V, filled slot 1].

*Close.*

26. Take the cheapest reversible step that passes your eval, and know that owning the harness's internals keeps you at rung 2 but makes every upgrade your problem.
...
27. At rung 3 the model is yours, which is what ML engineering meant before the generative era.

28. The received advice in 2026 is not to fine-tune, and it rests on real evidence.
...
29. Tuning wins where the right answer is not public.
...
30. On complex document extraction, tuned 3B and 8B models beat the strongest model our deployment allowed.
...
31. The cost of trying fell by an order of magnitude.
...
32. What you can train depends on what supervises the training, and budget cannot substitute for having the right signal.
    - Supervision table, four rows: demonstrations, preference pairs, teacher, verifier [EM §2].

33. Post-training is now several steps, and the newest step lets you train narrow specialists separately and merge them.
...
34. The eval you built at rung 2 is what makes rung 3 possible, because it becomes the training environment.
...
35. Labs now report agents doing real research work, and the people running those labs disagree in public about how fast to go.
...
36. Most self-improvement shipping today rewrites the harness, not the model weights, which puts its leading edge at rung 2.
    - AIDE², Ouroboros, Darwin Gödel Machine, Recursive [EM §3.2, §3.3].

37. It works exactly as far as a verifier reaches, and where there is a verifier, it overfits to it.
...
38. For a client, the practical lesson is that the scorer is the asset.
...
39. Place your problem on a rung by what you own and what is failing.
...
40. Two numbers set the operating point at rung 2, and most teams have neither: what a silent error costs, and the coverage-versus-error curve for their own data.

41. Enter rung 3 when prompting has plateaued and you have a training signal; volume decides whether it pays, not whether it works.

---

## Close

42. The numbers in this post will date within a year, and the structure should not.

43. **[YOU]** End on the failures actually witnessed and what they share: someone skipped measurement and went straight to the machinery.
...
- **43 paragraphs** at 150 to 200 words each gives roughly 6,500 to 8,500 words.
- **First-hand passages:** rung 1 context setup (11), context (20), verifiers (25), the extraction fine-tune (rung 3), and the GEPA overfitting note to place in self-improvement. Rung 1's result is the open one; see the author todo.
- **Material left out on purpose:** most survey statistics, most of the RSI papers, the full case-study table, the cost-per-task discussion. It stays in the notes.
- **Decisions to settle before drafting:** title; rung 1 result (14); whether the three-outcome paragraphs (16 to 19) become their own section; self-improvement length; paragraph 9.
