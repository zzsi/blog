...
- **Post-training is multi-stage.** Say the recipe: SFT for format, preference optimisation for alignment, verifiable-reward RL for reasoning, and sometimes a consolidation SFT after a large RL run as DeepSeek R1 does. **Do not write "DPO after RL"**; the recipes reviewed put DPO before RLVR, and the frontier is moving toward on-policy distillation instead. [SOURCE: emerging.md §2]
