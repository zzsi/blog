...
Three rungs, defined by what the client depends on.

| Rung | Name | Dependency | Service line |
|---|---|---|---|
| 1 | Set up to use agents well | Vendor defaults + your systems | Enablement, integration |
| 2 | Harness engineering | Frontier APIs you do not own | Engineering |
| 3 | Reduce frontier dependency | Weights you run | Fine-tuning, RL, serving |

Within each rung: **layers** (where you intervene). Within each layer: **instruct, configure, own** (how far you go). Three levels, each doing one job.
...
- Across generations the model dominates. Within the frontier tier, what you build decides how much of the ceiling you get. [REUSE]
- Thesis: the useful question is not which model, but **what you depend on**, and the answer changes what you build.
...
- **Candidate for the one idea an executive carries out:** a point of reliability is worth the cost multiple between human and agent. At 12x cheaper, one point of reliability is worth twelve points of unit price. Below roughly 80% reliability, rung 2 beats rung 3 whatever rung 3 promises. [SOURCE: business-value.md 2b]
...
## 4. Rung 1: set up so teams can use agents well (~1,200 words)
...
- **Why teams stall here:** most organizations stall between rung 1 and rung 2, not between 2 and 3. [REUSE]
- **[NEEDS: a first-hand story. The enablement program is the obvious source.]**

## 5. Rung 2: harness engineering (~2,800 words)

*The breadth-and-depth section. Still calling APIs you do not own.*

- **What it buys:** a dependable process that scales with volume. 70% is a demo; 95% at a known cost per run is a process. [REUSE]
- **Layer: measurement.** The money argument first: without a way to tell a success from a failure you either review everything, which throws away most of the prize, or review nothing, which loses outright once an escaped error costs more than a normal handling. A confidence policy is what turns reliability into realised saving. [SOURCE: business-value.md 2b, gap 1] Then three-level evals. Where the bottleneck actually sits is contested, and you cannot tell which reading applies to you without measuring. [REUSE: relocated bottleneck para] Verifiers fail in two directions. [REUSE: first-hand verifier story]
...
- **The escalation rule, stated once:** instruct, configure, own. Take the cheapest reversible step that clears your eval bar. The line between configuring and owning is the supported extension surface, not the file boundary. [REUSE: compressed classification test]
- **Owning harness internals lands here, not at rung 3** — you still depend on the same APIs. Carries maintenance, revalidation, upgrade tax. [REUSE: harness branch]

## 6. Rung 3: reduce dependency on frontier APIs (~1,800 words)
...
- fig3 J-curve — **needs rework**: relabel to the three rungs, and consider replacing it entirely. The dip now has a mechanism (dispersed unbankable savings, then the cost of not knowing which runs failed), so a figure showing *banked versus theoretical* value may beat an asserted curve shape.
