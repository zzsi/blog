...
Optimizers are easy to under-appreciate. We spend a lot of energy on architecture, data, and compute budgets, but the optimizer is the control policy that decides how noisy gradients become actual parameter updates.

Looking back through early 2026, optimizer progress did not follow a single clean line. It came in waves: acceleration, adaptivity, regularization corrections, systems scaling, and now geometry-aware updates.

In this post, I summarize that history and the practical defaults that emerged.
...
These ideas established the blueprint: smooth noisy gradients over time, respect curvature, and make updates less fragile to parameterization.
...
By the end of this period, the community had a clear direction: blend momentum-like smoothing with per-parameter scaling.
...
Another active thread in the same era was convergence behavior of Adam-family methods, with AMSGrad as a representative fix ([Reddi et al., 2018](https://openreview.net/forum?id=ryQu7f-RZ)).

AdamW's decoupled weight decay fixed this mismatch ([Loshchilov and Hutter, 2017](https://arxiv.org/abs/1711.05101)). That correction, simple on paper, became foundational for Transformer-era training recipes.
...
These methods solved real throughput bottlenecks, even if AdamW remained the broader long-term default.
...
The message was clear: the best optimizer is not just mathematically elegant; it has to fit hardware constraints.
...
The key shift is that conditioning is no longer only an optimizer-internals detail. It is becoming a central design axis for stability and scaling.

Community writeups and reproductions are useful for early signal, but should remain secondary evidence compared to controlled papers (for example: [HF community reproduction notes](https://huggingface.co/blog/bird-of-paradise/reproducing-and-validating-distributed-muon)).

## What actually won by 2025
...
LARS/LAMB remained valuable for specific extreme large-batch regimes rather than general defaults.
...
The strongest methods usually survive all three filters: coherent intuition, robust benchmarks, and practical systems fit.
...
By early 2026, AdamW remained the center of gravity for large language and multimodal models, but geometry-aware methods suggest the next wave may come from better directional control rather than only better scalar learning-rate heuristics.
