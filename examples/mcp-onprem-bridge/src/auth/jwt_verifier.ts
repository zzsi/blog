...
function parseScopes(payload: Record<string, unknown>): string[] {
  if (typeof payload.scope === "string") return payload.scope.split(" ").map((s) => s.trim()).filter(Boolean);
  if (Array.isArray(payload.scp)) return payload.scp.filter((v): v is string => typeof v === "string");
  return [];
}
    ...
    async verifyAccessToken(token: string) {
      const payload = options.mode === "shared_secret"
        ? (await jwtVerify(token, new TextEncoder().encode(options.secret), {
          algorithms: ["HS256"], issuer: options.issuer || undefined, audience: options.audience || undefined,
        })).payload
        : (await jwtVerify(token, createRemoteJWKSet(new URL(options.jwksUri)), {
          algorithms: ["RS256", "RS384", "RS512", "ES256", "ES384", "ES512"], issuer: options.issuer, audience: options.audience,
        })).payload;
