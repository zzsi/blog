...
All the adaptive methods above (AdaGrad, RMSProp, Adam) are already doing *preconditioning*: transforming the gradient before stepping. Instead of $\Delta\theta = -\eta\, g$, they use $\Delta\theta = -\eta\, P\, g$ for some matrix $P$ that reshapes the step to account for the loss surface's geometry. In Adam, $P$ is diagonal, with entries $1/\sqrt{\hat{v}_t}$ for each parameter. This adjusts the step size per parameter but ignores how parameters interact.
...
Neither became a broad default. The implementation complexity and tuning overhead exceeded what most teams would absorb. But they proved that accounting for cross-parameter structure could outperform Adam's independent scaling. That lineage matters: the 2024-2026 conditioning wave reuses the same motivation with simpler operational surfaces.
...
LARS and LAMB themselves did not become standard in frontier LLM or diffusion model training. The community settled on AdamW with careful scheduling instead. The specific legacy of this work is narrower: **the insight that different layers need different effective learning rates**. Muon and the conditioning methods in the next sections address this same per-layer imbalance, just through orthogonalization rather than trust ratios.

(Two other ingredients now standard in frontier training, learning rate warmup and gradient clipping, are sometimes associated with large-batch research but have their own origins: warmup was already used in the original Transformer paper ([Vaswani et al., 2017](https://arxiv.org/abs/1706.03762)), and gradient clipping was introduced for exploding gradients in RNNs ([Pascanu et al., 2013](https://arxiv.org/abs/1211.5063)).)
