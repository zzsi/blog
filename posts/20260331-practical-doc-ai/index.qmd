...
![Automation coverage versus error rate. Raising coverage means automating less certain cases, so errors stay flat for a while and then climb steeply. The confidence policy chooses where on this curve the business operates. Illustrative curve, not measured data.](coverage-vs-error.svg){width=700px fig-align="center"}
...
To see a template-registration-OCR pipeline in action, with synthetic corruptions that show exactly where it breaks, visit [Room 1 of the Doc AI Museum](museum/room1-templates.qmd).
...
![The detector-first pipeline. Each stage is trained or tuned on its own, and each handoff is a place where an upstream mistake becomes a downstream failure that nothing catches.](detector-pipeline.svg){width=900px fig-align="center"}
...
![A layout-aware transformer in the style of LayoutLMv3. Image patches, OCR tokens, and each token's 2D position are embedded and attended jointly, and the output is a per-token decision about whether that token is part of the answer. Supervision comes from labeling the final answer and projecting it back onto tokens.](layout-transformer.svg){width=900px fig-align="center"}
...
![Deterministic pipeline versus agentic control flow. On the left, a hand-authored fixed sequence. On the right, the model holds the context and decides which tools to call and in what order, then commits or escalates based on agreement and validators.](deterministic-vs-agentic.svg){width=900px fig-align="center"}
