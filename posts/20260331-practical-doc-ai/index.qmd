...
I do not mean that the model is left unconstrained. The tools, schemas, validators, and review policy are still designed by us. But the extraction flow itself no longer has to be a rigid hand-authored pipeline. For messy documents, that flexibility matters. The model can decide to inspect one page more closely, compare OCR output with visual evidence, call retrieval utilities for context, or reconcile conflicting intermediate results before committing to an answer.
