...
I've worked on document AI problems for about a decade. The business problem stayed mostly the same. The stack did not.

The core problem: extract the right information from messy documents, map it into a schema, make the result reliable enough for a real workflow. Sometimes contracts. Sometimes receipts, forms, statements, onboarding packets, tax documents. What changed was the modeling stack.

It started with templates, image registration, and OCR. Then convnet-based detector-plus-OCR pipelines. Then transformer models that combined OCR text, layout, and image cues in one trainable system. Now the stack often includes OCR, VLMs, LLMs, validators, retrieval, post-training, and increasingly agentic tool use.

Each phase mostly moved the bottleneck. That's the pattern I want to capture here.

::: {.callout-note appearance="simple"}
## TL;DR

- Each generation of document AI mostly moved the bottleneck rather than eliminating it: from template brittleness, to pipeline complexity, to labeled data hunger, to orchestration and evaluation.
- Foundation models changed the economics but didn't replace the system. The hard part shifted from "can the model read this" to "can we trust the answer."
...
| Detector-first pipelines | Object detectors + OCR + postprocessing | Better visual robustness and localization | Labeling burden, detector-OCR handoff, error propagation |
| Transformer models | OCR text + layout + image cues in one model | Easier supervision, stronger field extraction | Task-specific data prep, specialized models, single-page limits |
...
## The problem was never just OCR

People sometimes talk about document AI as if it were synonymous with OCR. It never was.

In practice, the business problem looked more like this:
...
- validate that extracted values are internally consistent
- decide when to ask for human review

That last point matters more than benchmark papers suggest. A document system is only useful when it knows when not to trust itself.

## How we measured success

The metric that mattered most was usually field-level accuracy, but defining "correct" was harder than it sounds. Addresses can be broken into lines multiple valid ways. Dates appear in multiple formats. OCR introduces harmless whitespace differences. So evaluation always needed normalization logic and task-aware matching, not naive string equality.
...
- among the automated cases, what's the error rate

That's a policy problem as much as a modeling problem. The automation policy depends on confidence signals. If confidence estimation is poor, the policy is poor even if the raw model is good.

I came to think about evaluation in layers: field-level accuracy, automation coverage, automation precision, review rate, and how all of those move as confidence thresholds change. The most important chart was often not a single benchmark score but a coverage-versus-error curve.

Depending on the task, standard supporting metrics also mattered:

- CER and WER for OCR quality
- entity-level precision, recall, and F1 for key information extraction
- [ANLS](https://arxiv.org/abs/1907.00490) for [DocVQA](https://www.docvqa.org/)-style tasks where near-matches deserve partial credit
...
That's why I'm skeptical of one-number evaluations in document AI. The system isn't trying to be correct in the abstract. It's trying to support a decision policy.

## Phase 1: templates, OCR, and heuristics

My early mental model was straightforward: if a document class is stable enough, you can solve a lot with templates, image registration, OCR, and rules.
...
In a constrained workflow, this worked better than many people remember. If the document family was narrow, the scan quality acceptable, the registration reliable, and the downstream schema stable, template-heavy systems could be fast, cheap, and surprisingly accurate.
...
That phase taught me the hard part wasn't reading text. It was handling variation without rewriting the system every time a document changed shape.

## Phase 2: detector-first pipelines

The next improvement came from better visual models. Instead of assuming we knew where everything was, we trained convnet-based detectors to find regions of interest: tables, signatures, key-value fields, checkboxes, headers.

Even a moderately good detector could absorb variability that broke template-only systems. You could ask a model to find a field rather than hard-coding its location.
...
It was multimodal in a loose engineering sense, but not in a unified learned sense. Vision handled localization. OCR handled text recognition. Rules handled interpretation.

That decomposition was often right for the tools of the time, but it came with painful handoff boundaries. If the detector missed the box, OCR never had a chance. If OCR garbled the crop, the parser failed. If table structure was slightly off, everything downstream became an exception case.

Better perception didn't automatically mean simpler systems. In many cases it meant the opposite.
...
The models were larger, but the cost was acceptable. In practice, model inference time was still small compared with the rest of a real production pipeline: retrieving PDFs from data sources, querying databases, and doing all the ordinary glue work around the model.
...
Another important change: [agentic](https://arxiv.org/abs/2210.03629) control flow. In older systems, we wired steps together deterministically. In newer systems, we increasingly provide the model with context and tools and let it decide which steps to invoke and in what order.
